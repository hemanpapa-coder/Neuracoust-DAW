...
## Goal
Reproduce the Codex Design look as a **native macOS app**, driven by the **same realtime audio engine** we already built. Keep engine and UI as separate layers so both can keep evolving independently.
...
so the glyph changes as the mode does — designed in the Codex Design project
